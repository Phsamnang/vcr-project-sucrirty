...
public interface SaleRepository extends JpaRepository<Sale,Long> {
    ...
    @Query("select distinct s from Sale s where s.tableSale.id = ?1 and s.tableSale.status = ?2 and s.id=?3")
    Sale findByTableSale_IdAndTableSale_Status(Long id, String status,Long saleId);
